...
def perfTest():
    ...
    run_time_tot = 300 # total iperf3 runtime, in seconds. I recommend more than 300 sec.

    h1, h2, h3, h4 = net.get('h1','h2','h3','h4')
    

    # h3 ping h1
    h3.cmd('ping 10.0.0.4 -i 1 -c %d > h3_ping_result_%s.txt &' % (run_time_tot, str(myQueueSize)))

    # Receiver h2
    h2.cmd('iperf3 -s -i 1 > h2_server_%s.txt &' % (str(myQueueSize)))
    # Receiver h4
    h4.cmd('iperf3 -s -i 1 > h4_server_%s.txt &' % (str(myQueueSize)))
    ...
    h1.cmd('iperf3 -c 10.0.0.2 -t %d -C %s > flow1_%s_%s.txt &' % (run_time_tot, TCP_TYPE_first, TCP_TYPE_first, str(myQueueSize)))
    ...
    h1.cmd('iperf3 -c 10.0.0.4 -t %d -C %s > flow2_%s_%s.txt &' % (run_time_tot, TCP_TYPE_second, TCP_TYPE_second, str(myQueueSize)))
